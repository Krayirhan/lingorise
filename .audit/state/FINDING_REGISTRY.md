...
| CORE-004 | P1 | Core correctness | CLOSED | RUN-006-REAUDIT | RUN-007-REAUDIT | Daily-practice hero CTA silently no-op'ladığı, level'ın tamamen öğrenildiği terminal state artık her iki ekranda da (Home/GardenHeroCard, PracticeHubScreen) doğru state/CTA gösteriyor; startPractice/startExam artık boolean dönüyor (sessiz dead-end yok); useHomeViewModel'in greeting-text dalındaki aynı sınıf ikinci bir kopya (RUN-007'nin kendi re-review'unda bulundu) de düzeltildi. 342/342 test, 0 tsc hatası. Kapatılma working tree üzerinden (henüz commit edilmedi) — acceptance criteria unit-test yoluyla karşılandı, on-device/Maestro pass henüz yapılmadı (disclosed gap). |
