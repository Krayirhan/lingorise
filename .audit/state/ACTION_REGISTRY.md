...
| ACT-DATA-001 | P1 | DONE | DATA-001 | RUN-001-BASELINE | RUN-004-REAUDIT (partial — see note) | Eliminate cold-start local-storage race; unit test added (testSuite.ts #56), physical two-device verification still pending |
...
| ACT-ACC-001 | P3 | DONE | ACC-001 | RUN-001-BASELINE | RUN-004-REAUDIT (4/5 DoD — see note) | Onboarding + LevelPromotionModal + reduceMotion scope closed (commit 55130bf); remaining: Accessibility Scanner app run, Profile/Progress/Auth TalkBack pass |
