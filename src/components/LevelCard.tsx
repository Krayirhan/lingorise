...
export function LevelCard({ level, selected, onPress }: { level: Level; selected: boolean; onPress: () => void }) {
  return <Pressable style={[styles.card, selected && styles.selected]} onPress={onPress}><View style={[styles.badge, selected && styles.badgeSelected]}><Text style={styles.code}>{level.code}</Text></View><View style={styles.copy}><Text style={styles.title}>{level.title}</Text><Text style={styles.hint}>{level.hint}</Text></View><Ionicons name="chevron-forward" size={20} color="#A9A39A" /></Pressable>;
}
