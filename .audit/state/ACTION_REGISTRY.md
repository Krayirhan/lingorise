...
| ACT-CORE-004 | P1 | VERIFIED | CORE-004 | RUN-006-REAUDIT | RUN-007-REAUDIT | Gated PracticeHubScreen + GardenHeroCard hero CTAs on isLevelFullyLearned; startPractice/startExam return boolean with AppNavigator toast fallback; fixed reused "come back tomorrow" copy in both practiceRecommendation and (found during RUN-007's re-review) the separate greetingTitle/Subtitle branch. 14 regression assertions (testSuite.ts Birim 58). Verified via unit/source tests per CORE-004's own acceptance criteria; on-device pass still pending (disclosed) |
