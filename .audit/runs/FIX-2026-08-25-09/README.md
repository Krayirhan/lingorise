...
## Round 9 — self-inflicted regression, caught and corrected
While editing `.maestro/smoke.yaml` to wrap the `Pratik` tab selector (end of Round 8), the edit accidentally deleted the entire `tapOn: ".*Hemen Başla.*"` line. Run `32895871429` failed at `Tap on "Seviyemi seç"` never reaching the ready screen — caught by re-reading the file after the log showed a step sequence skipping straight past where that tap should have run. Fixed by re-adding the line; committed with an explicit self-correction note rather than folding it silently into the next change.

## Round 10 — native permission dialog theory (partially right, not sufficient)
Run `32895871429` (line restored) progressed further but still failed at the final `assertVisible`. The downloaded debug screenshot showed the app stuck on `ReadyStep` (4/4) despite the `"Hemen Başla"` tap reporting `COMPLETED`. Theory: `ReadyStep`'s guest button has the daily-reminder toggle on by default, so tapping it calls `scheduleDailyReminder() → Notifications.requestPermissionsAsync()`, which on Android 13+ shows a native system dialog outside the React tree, invisible to any RN selector, blocking the flow. Added an optional `tapOn: {text: "Allow|İzin Ver|İzin ver", optional: true}` step. Run `32897402511`: the optional tap reported `WARNED` (no matching dialog found), yet the `"Pratik"` tap still reported `COMPLETED` and the final assertion still `FAILED` — so this fix alone didn't resolve it, and the true mechanism needed more evidence.

## Round 11 — the true, final root cause: a colliding substring match, not a permission dialog
Downloaded and inspected run `32897402511`'s full debug artifact directly — both saved screenshots (`step-010`, `step-012`, the only two Maestro auto-saves since it only persists artifacts for non-`COMPLETED` steps) showed the app **still sitting on `ReadyStep` (4/4)** at both checkpoints, confirming the flow never actually left that screen despite `"Hemen Başla"` and `"Pratik"` both reporting `COMPLETED`.

`maestro.log` (host-side driver log, not the device logcat) recorded the literal tap coordinates for every step. For `.*Hemen Başla.*`: `Tapping at (540, 931)`. The real button's bounds (from the saved hierarchy JSON) are `[53,1938][1028,2074]` — center ≈ `(540, 2006)`. **Nowhere close.** Coordinate `(540, 931)` lands on this same screen's own headline, `"Hesap açmadan hemen başlayabilirsin."` — and `.*Hemen Başla.*` matches it too: Maestro's `textRegex` is case-insensitive, and the literal substring `"hemen başla"` is a prefix of `"hemen başlayabilirsin"`. Every run since Round 8's wildcard fix had been tapping a plain, non-interactive `Text` node instead of the button — a silent no-op that Maestro still reports as `COMPLETED` (it only verifies the `adb input tap` command executed, not that anything happened in the app). This also fully explains the `"Pratik"` tap's behavior on this same stalled screen: its accessible label collided with the Switch's `accessibilityLabel="Düzenli pratik yapmak ister misin?"` (contains "pratik", case-insensitive), which is why the reminder toggle visibly flips ON between the two saved screenshots even though the flow never navigated anywhere. The earlier permission-dialog theory (Round 10) was reasonable given the evidence available at the time, but was never the actual blocker — the button tap was failing before any dialog could even matter.

### Real fix
`.maestro/smoke.yaml`: changed the selector from `.*Hemen Başla.*` to `.*Misafir Olarak.*` — a substring that appears only once on screen, on the real button (`"Hemen Başla (Misafir Olarak) →"`), confirmed via the saved hierarchy JSON before committing. Also verified `.*Pratik.*` is safe on the actual home screen it targets: the only other copy string containing "pratik" (`heroPracticeAgain: "Tekrar pratik yap"`) is gated behind `isCompleted` in `GardenHeroCard.tsx` and never renders for a fresh guest session, so no collision risk there.

**Verified**: run `32899393092` (commit `1b616e0`) — `verify`, `android-build`, and `e2e-smoke` **all passed**. https://github.com/Krayirhan/lingorise/actions/runs/32899393092

## Status
`VERIFIED` / `CLOSED`. Eleven real, distinct, evidence-confirmed issues were found and fixed across this FIX entry's rounds: missing JDK 21 for firebase-tools, `gradlew` not executable, three stale Maestro selectors, missing `.env`, a debug build with no embedded JS bundle, an unbounded Firestore call in `catalogueService.ts`, an unbounded `onAuthStateChanged` wait in `AppBootstrap.tsx`, Maestro's full-match `textRegex` semantics requiring explicit wildcards, a self-inflicted deleted step (caught and corrected), a native-permission-dialog theory that was reasonable but insufficient, and the true final cause — a case-insensitive substring collision between a wildcarded selector and this screen's own headline text, causing a silent no-op tap on plain text instead of the real button. Every one was verified via direct evidence — a real CI log, a real downloaded screenshot, Maestro's own hierarchy dump, `maestro.log`'s literal tap coordinates, or a real logcat — never assumed from a plausible-sounding theory alone; intermediate theories that later proved insufficient (cold-start timing in Rounds 4-6, the permission dialog in Round 10) were each tried, found wanting by further evidence, and corrected rather than left standing. The two app-level reliability fixes (`catalogueService.ts` and `AppBootstrap.tsx` timeouts) are kept regardless of whether they were the CI blocker — genuine product improvements against a real class of bug (unbounded network waits with no fallback) that a real user on bad network could also hit.
...
## Final evidence
- Passing run: https://github.com/Krayirhan/lingorise/actions/runs/32899393092 (commit `1b616e0`)
- All 3 jobs (`verify`, `android-build`, `e2e-smoke`) green in the same run.
